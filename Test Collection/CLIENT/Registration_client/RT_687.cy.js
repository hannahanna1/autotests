...
describe("test_687", () => {
  it("Registration with password < 7 characters (Client)", () => {
    ...
    //input Password
    cy.get('[placeholder="Password"][type="password"]').type("1234abc");
    cy.get(".ant-form-item-children-icon")
      .eq(1)
      .should("have.css", "color", "rgb(255, 77, 79)");

    // confirm password
    cy.get('[id="confirmPassword"]').type("1234abc");
    cy.get(".ant-form-item-children-icon")
      .eq(2)
      .should("have.css", "color", "rgb(82, 196, 26)");
    ...
    cy.get('[class="ant-form-item-explain-error"]')
      .should("be.visible")
      .and("have.text", "Minimum length: 8 characters")
